...
class BITMAP(Structure):
  ...
  def __init__(self, stream, offset, max_size, parent, name, \
    ...
    if image_data_offset == None:
      # If no offset is specified, assume the pixel data comes immediately
      # after the struct_BITMAPINFO:
      image_data_offset = self.current_offset;
...
#          'uneven: rounded up to 0x%X|%d' % (w, w));
  ...
  def DecodeRLE(self, bits, data, w, h):
    ...
    x = 0; y = 0; index = 0;
#    max_x = 0; max_y = 0;
    data.ContainStream('rle_decompressed_pixel_data', \
        'rle_compressed_pixel_data', '');
...
#        y += ord(data.value[index]);
        ...
        data.ContainStream('rle_decompressed_pixel_data', 
            'rle_compressed_pixel_data', \
            data.value[index:index + size], size * bits / 8);
        index += size;
        x += size;
#      if x > max_x:
#        max_x = x;
